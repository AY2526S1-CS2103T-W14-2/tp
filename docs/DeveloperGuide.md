...
1. Make `timing` input format more flexible. The only current format accepted is `HH:mm`. We plan to accept multiple different formats for `timing` such as `HHmm`, `HH mm`, etc.
2. Make `opening time` and `closing time` to be separated. The current validation for `timing` requires both opening and closing time to be provided together. It is possible for different `Foodplaces` to have uniform `opening time` but not a uniform `closing time` (e.g. if a `Foodplace` sells out early, it would be good to have the flexibility to only add `opening time` for such scenarios). We plan to allow users the flexibility to provide only one part of it or both.
